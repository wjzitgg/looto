<template>
    <transition>
        <div ref="dragIcon"
            ...
            @click="$emit('clickBall')"
            :style="{left: left + 'px',top: top + 'px',width: itemWidth + 'px',height: itemHeight + 'px'}"
          
            v-if="isShow">
            <i class="iconfont icondownload-fill"></i>
                <p class="dragIcon-a">Download</p>
                <p class="dragIcon-b">APP</p>
        </div>
    </transition>
</template>
...
.dragIcon {
    ...
    border-radius: 50%;
    background-color: #d64351;
    
    text-align: center;
    color: #fff;
    
    box-shadow: 0px 0px 10px -2px #803fc5;
    z-index: 99999;
    display: flex;
    flex-wrap: wrap;
    align-items: center;
